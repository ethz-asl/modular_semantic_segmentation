...
def loglikelihood_logp(logp, N, a, delta=1e-2):
    '''Compute log likelihood of Dirichlet distribution, i.e. log p(D|a).

    Parameters
    ----------
    logp : sufficient statistic for D
    a : array
        Parameters for the Dirichlet distribution.
...
def _meanprecision(D, tol=1e-7, maxiter=None):
    ...
    if maxiter is None:
        maxiter = MAXINT
    for i in xrange(maxiter):
        a1 = _fit_s(a0, logp, tol=tol)
        s1 = sum(a1)
        a1 = _fit_m(a1, logp, tol=tol)
        m = a1/s1
        # if norm(a1-a0) < tol:
        if abs(loglikelihood(D, a1)-loglikelihood(D, a0)) < tol: # much faster
            return a1
...
def fixedpoint_with_sufficient_statistic(ss, N, K, prior, tol=1e-7, maxiter=1000,
                                         delta=1e-2):
    '''Simple fixed point iteration method for MLE of Dirichlet distribution'''
    logp = ss
    a0 = prior
...
def _fit_s(a0, logp, tol=1e-7, maxiter=1000, delta=1e-2):
    ...
    raise Exception('Failed to converge after {} iterations, s is {}, change is {}'
            .format(maxiter, s1, abs(s1 - s0)))

def _fit_m(a0, logp, tol=1e-7, maxiter=1000):
    ...
    raise Exception('Failed to converge after {} iterations, s is {}'.format(maxiter, s))
